...
\begin{document}
	...
	Urban traffic congestion imposes substantial economic and social costs on cities worldwide, with the burden falling disproportionately on rapidly urbanizing regions in Southeast Asia. As vehicle registration rates outpace road network expansion, the optimization of existing infrastructure, particularly traffic signal control at key intersections, has emerged as a cost-effective lever for improving mobility without the prohibitive expense of road construction. In the Philippines, the 2024 TomTom Traffic Index ranks Metro Manila among the most congested cities in Asia, and secondary urban centers including Davao City, Cebu City, and Quezon City exhibit similar patterns of peak-hour gridlock and signal-induced delay [1]. The proliferation of motorcycles, jeepney-type public utility vehicles, and goods trucks alongside passenger cars creates a heterogeneous traffic stream with wide variation in vehicle dynamics, space occupancy, and saturation flow rates, characteristics that strain the assumptions underlying conventional fixed-time signal timing models [2].
	
	The dominant approach to traffic signal control in Philippine cities remains fixed-time operation, in which pre-programmed cycle lengths and phase splits are calibrated from historical volume data and deployed without real-time adaptation. While computationally simple, these systems are inherently static: they cannot respond to fluctuations in demand caused by incidents, special events, or progressive changes in travel patterns [3]. Vehicle-actuated and semi-actuated controllers offer incremental improvements by extending green phases in response to detector inputs, but they optimize locally within individual phase cycles and do not accumulate knowledge of long-term traffic patterns. More sophisticated adaptive systems such as SCOOT (Split Cycle Offset Optimization Technique) and SCATS (Sydney Coordinated Adaptive Traffic System) have demonstrated measurable improvements in coordinated arterial conditions but require substantial infrastructure investment, continuous detector maintenance, and expert calibration efforts that remain impractical for many intersections in developing-country contexts [4].
	
	Evolutionary computation, and Genetic Algorithms (GAs) in particular, offers a compelling alternative for offline signal timing optimization. GAs search the space of feasible timing plans through biologically inspired operators of selection, crossover, and mutation, evaluating candidate solutions via simulation rather than requiring a closed-form objective function [5]. This property makes GAs especially attractive when the relationship between signal timing parameters and traffic performance is nonlinear, non-convex, or dependent on vehicle class interactions that are difficult to model analytically. By coupling a GA optimizer with high-fidelity microscopic simulation in SUMO (Simulation of Urban MObility), it is possible to evolve signal timing plans that account for realistic vehicle dynamics, including the slow acceleration of loaded goods trucks, the high density of motorcycles in shared lanes, and the variability in driver behavior, using empirical demand data as simulation input [6].
	
	Despite a growing body of literature on GA-based signal optimization in international contexts, comparative applications using locally collected DPWH traffic classification data at specific Philippine urban intersections remain scarce. Existing analytical timing tools calibrated for passenger car equivalent assumptions may systematically underestimate the impact of the Philippines' distinctive vehicle mix on saturation flow and phase capacity, creating a practical gap between internationally derived timing recommendations and locally appropriate plans [7].

	Davao City was selected as the study site for three practical reasons. First, it is the largest city in Mindanao and among the fastest-growing secondary urban centers in the Philippines, with documented signal timing inflexibility at major arterials identified as a principal driver of intersection delay in existing local research [7]. Second, DPWH traffic classification data at key corridor sites in the city is available, enabling empirical demand modeling without reliance on synthetic or stylized traffic profiles. Third, the existing fixed-time plans at the study intersections have not been subjected to any documented microsimulation review, making them strong candidates for simulation-coupled optimization.

	This study addresses this gap by applying a GA-based signal timing optimizer to two signalized intersections in Davao City --- E.~Quirino Avenue and the Quimpo Boulevard--GE Torres intersection --- using vehicle demand and type distributions derived from five-day DPWH traffic classification surveys conducted in April 2025. All optimization and evaluation is conducted within SUMO microsimulation models calibrated to the physical geometry, existing phase sequences, and vehicle mix of both sites. The specific objectives of this study are as follows:
	...
	The persistent deployment of fixed-time plans at intersections such as E.~Quirino Avenue and Quimpo--GE Torres, where current green splits of 95~s to 45~s (Quirino, 150~s cycle) and multi-phase plans of 250~s cycle length (Quimpo) were set without documented optimization using microsimulation feedback, underscores the practical relevance of accessible offline optimization approaches. A GA-based optimizer that requires only a calibrated simulation model and historical volume data is directly applicable in this context.
	...
	It is worth positioning GA-based offline optimization against two alternative approaches that have appeared in recent literature. Reinforcement learning (RL) agents, trained via repeated interaction with a simulation environment, can adapt signal control decisions in real time and have demonstrated strong performance in benchmark environments. However, RL requires extensive training episodes over thousands of simulation steps, careful reward function design, and substantial computational resources before deployment-ready policies emerge; furthermore, the resulting control policy is implicit in a neural network rather than a human-interpretable timing plan, limiting its practical adoption on conventional signal controllers. Fuzzy logic controllers offer interpretable rule-based adaptation, but their performance depends on expert-crafted membership functions that do not generalize readily across intersections with different geometry or vehicle mix and require manual re-tuning when traffic patterns change. GA-based offline optimization, by contrast, requires only a calibrated simulation model and historical demand data, produces a fixed timing plan directly programmable on existing signal hardware, and imposes no real-time sensing infrastructure --- making it the most operationally practical choice for the Philippine intersections studied here, where adaptive infrastructure is unavailable.
	...
	Traffic congestion in Philippine urban centers has received growing scholarly and policy attention as vehicle registration growth outpaces infrastructure expansion. The TomTom Traffic Index [1] ranks Metro Manila among the most congested cities in Asia, with average commuters spending more than 40 additional minutes per hour during peak periods. While secondary cities including Davao City, Cebu, and Quezon City report lower absolute congestion indices than the capital, their rates of vehicle fleet growth and the absence of mass rapid transit alternatives suggest that these centers will face compounding signal management challenges over the medium term.
	...
	The literature reviewed above establishes that Genetic Algorithms offer a tractable and well-validated approach to traffic signal timing optimization, particularly when coupled with microscopic simulation for fitness evaluation. GA-based methods have consistently outperformed Webster's analytical formula under peak and variable demand conditions, demonstrated robustness to demand uncertainty, and successfully addressed arterial coordination problems. SUMO provides an open-source, reproducible, and scriptable simulation platform that has been widely adopted for traffic control research and supports the vehicle type diversity required to model Philippine heterogeneous traffic accurately.
