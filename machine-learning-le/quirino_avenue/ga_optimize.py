"""
Genetic Algorithm optimizer for E. Quirino Avenue traffic signal timing.
...
Chromosome: 8 integers [g0..g7], one green-phase duration per controllable green
phase across 4 traffic lights. Yellow inter-greens (5 s) and all-red phases are fixed.

Usage:
    python ga_optimize.py
Outputs:
    ga_results.csv              — best fitness per generation
    ga_results_best_chromosome.csv — winning timing plan
"""
...
import traci  # noqa: E402  (env var must be set before import)

# ---------------------------------------------------------------------------
# Paths (relative to this file)
# ---------------------------------------------------------------------------
_DIR        = os.path.dirname(os.path.abspath(__file__))
NET_FILE    = os.path.join(_DIR, 'quirino_avenue.net.xml')
ROUTE_FILE  = os.path.join(_DIR, 'traffic_routes.rou.xml')
VTYPE_FILE  = os.path.join(_DIR, 'vehicle_types.add.xml')
TTL_FILE    = os.path.join(_DIR, 'quirino_avenue.ttl.xml')
...
OMEGA           = 0.5   # queue-length penalty weight in fitness function
EPSILON         = 1e-6  # singularity guard
NUM_SECONDS     = 86400 # 24-hour simulation episode

# ---------------------------------------------------------------------------
# Traffic light phase configuration (derived from quirino_avenue.ttl.xml)
#
#   green_indices: 0-based indices of optimisable (green) phases in the TL program
#   baseline:      current fixed-time plan durations for those phases
#
# Yellow (5 s) and all-red phases are at the remaining indices and are fixed.
# ---------------------------------------------------------------------------
TL_CONFIGS = {
    '259606750': {'green_indices': [0, 2], 'baseline': [95, 45]},
    '259608478': {'green_indices': [0, 2], 'baseline': [95, 25]},
    '267530736': {'green_indices': [0, 2], 'baseline': [95, 45]},
    '267530738': {'green_indices': [0, 2], 'baseline': [95, 25]},
}

BASELINE_CHROMOSOME = [d for cfg in TL_CONFIGS.values() for d in cfg['baseline']]
CHROMOSOME_LENGTH   = len(BASELINE_CHROMOSOME)  # 8
...
def run_simulation(chromosome, trial_id=0):
    ...
    while traci.simulation.getTime() < NUM_SECONDS:
        ...
        if traci.simulation.getMinExpectedNumber() == 0:
            break  # all vehicles finished early
...
def run_ga():
    ...
    print(f"GA optimisation — E. Quirino Avenue")
